...
// FastAPI's own request validation (as opposed to our own HTTPException calls, which always send
// a plain string) returns `detail` as an array of {loc, msg, type} objects. Rendering that
// directly via `new Error(payload.detail).message` stringifies the array to "[object Object]" —
// this turns either shape into an actual readable sentence.
const errorMessage = payload => {
  const detail = payload?.detail;
  if (typeof detail === "string") return detail;
  if (Array.isArray(detail)) return detail.map(item => item?.msg ? `${(item.loc || []).slice(-1)[0] || "field"}: ${item.msg}` : JSON.stringify(item)).join("; ");
...
function App() {
  ...
  const generate = async (record = null) => {
    ...
    try {
      ...
      const payload = await response.json(); if (!response.ok) throw new Error(errorMessage(payload) || "Speech generation failed.");
...
function Generations({ items, voices, onRestore, onRegenerate, onDelete }) { const [query,setQuery]=useState(""); const q=query.toLowerCase(); const filtered=items.filter(item => item.text.toLowerCase().includes(q) || (item.benchmark_label || "").toLowerCase().includes(q) || voices.find(v=>v.id===item.voice_id)?.name.toLowerCase().includes(q)); return <><label className="search"><Icon name="search"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search label, text, or voice…"/></label>{filtered.length ? <div className="generation-list">{filtered.map(item => <article key={item.id}><button className="play" onClick={() => new Audio(assetUrl(item.audio_url)).play()} aria-label={`Play ${item.benchmark_label || "take"}`}><Icon name="play"/></button><div className="generation-copy">{item.benchmark_label && <em className="benchmark-label">{item.benchmark_label}</em>}<strong>{item.text}</strong><span>{voices.find(v=>v.id===item.voice_id)?.name || item.voice_id} · {formatDate(item.created_at)} · {item.language} · {Number(item.speed).toFixed(2)}×{item.performance ? ` · ${item.performance} preset` : ""}</span></div><div className="generation-readout"><span>{formatTime(item.duration_seconds)}</span><b>{item.generation_seconds.toFixed(2)}s GEN</b></div><div className="row-actions"><button onClick={() => onRestore(item)}>Restore</button><button onClick={() => onRegenerate(item)}><Icon name="refresh"/></button><button className="danger" onClick={() => onDelete(item)}><Icon name="trash"/></button></div></article>)}</div> : <Empty icon="history" title={items.length ? "No matching takes" : "No generations yet"} body={items.length ? "Try a different label, text, or voice search." : "Rendered speech will become a real, playable local library here."}/>}</> }

function Compare({ items, voices }) { const [a,setA]=useState(items[0]?.id||""), [b,setB]=useState(items[1]?.id||""); const A=items.find(i=>i.id===a), B=items.find(i=>i.id===b); return items.length >= 2 ? <div className="compare-grid">{[["A",A,setA],["B",B,setB]].map(([label,item,setter]) => <section className="compare-side panel" key={label}><div className="channel">CHANNEL {label}</div><select value={item?.id||""} onChange={e=>setter(e.target.value)}>{items.map(g=><option key={g.id} value={g.id}>{g.benchmark_label || `${voices.find(v=>v.id===g.voice_id)?.name || g.voice_id} — ${g.text.slice(0,42)}`}</option>)}</select>{item && <>{item.benchmark_label && <em className="benchmark-label">{item.benchmark_label}</em>}<Waveform/><audio controls src={assetUrl(item.audio_url)}/><p>“{item.text}”</p><div className="compare-meta"><span>{item.model_id}</span><span>{formatTime(item.duration_seconds)}</span><span>{item.generation_seconds.toFixed(2)}s gen</span></div></>}</section>)}</div> : <Empty icon="compare" title="Two real takes are required" body="Generate at least two takes. Compare never invents ratings, scores, or placeholder audio."/> }
...
function CreateVoice({ onClose, onCreated }) { const [name,setName]=useState(""),[transcript,setTranscript]=useState(""),[files,setFiles]=useState([]),[authorized,setAuthorized]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""); const submit=async e=>{e.preventDefault();if(!name.trim()||!files.length||!authorized)return setError("Add a name, at least one audio file, and confirm permission.");setBusy(true);const form=new FormData();form.append("name",name.trim());form.append("reference_text",transcript.trim());form.append("authorization_acknowledged",String(authorized));files.forEach(file=>form.append("files",file));try{const response=await fetch(apiUrl("/voices"),{method:"POST",body:form});const payload=await response.json();if(!response.ok)throw new Error(errorMessage(payload)||"Voice creation failed.");onCreated(payload.voice)}catch(err){setError(err.message)}finally{setBusy(false)}}; return <div className="modal"><form className="dialog" onSubmit={submit}><header><div><p className="label">NEW AUTHORIZED PROFILE</p><h2>Patch in a voice</h2></div><button type="button" onClick={onClose} aria-label="Close"><Icon name="close"/></button></header><p>Use a clean, single-speaker recording. Source files stay on this machine.</p><label>VOICE NAME<input value={name} onChange={e=>setName(e.target.value)} maxLength="80" placeholder="Studio name"/></label><label>REFERENCE TRANSCRIPT <span>recommended</span><textarea value={transcript} onChange={e=>setTranscript(e.target.value.slice(0,2000))} maxLength="2000" placeholder="Words spoken in the sample"/></label><label className="upload"><Icon name="upload"/><span>{files.length ? `${files.length} file${files.length>1?"s":""} selected` : "Choose reference audio"}<small>WAV, MP3, M4A, FLAC, OGG, AAC or WebM</small></span><input type="file" accept="audio/*" multiple onChange={e=>setFiles(Array.from(e.target.files||[]))}/></label><label className="consent"><input type="checkbox" checked={authorized} onChange={e=>setAuthorized(e.target.checked)}/><span>I own this voice or have explicit permission to reproduce it.</span></label>{error&&<p className="form-error">{error}</p>}<footer><button type="button" onClick={onClose}>Cancel</button><button className="primary" disabled={busy}>{busy?"Preparing…":"Save voice"}</button></footer></form></div> }

function PerformanceDialog({ voice, performances, onClose, onCreated, onDeleted }) { const presets=["neutral","warm","playful","serious","soft","excited","concerned","firm","intimate","tired"]; const available=presets.filter(item=>!performances.some(existing=>existing.preset===item)); const [preset,setPreset]=useState(available[0]||""),[transcript,setTranscript]=useState(""),[file,setFile]=useState(null),[authorized,setAuthorized]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""); const submit=async e=>{e.preventDefault();if(!preset||!transcript.trim()||!file||!authorized)return setError("Choose a performance, add its exact transcript and audio, and confirm permission.");setBusy(true);const form=new FormData();form.append("preset",preset);form.append("reference_text",transcript.trim());form.append("authorization_acknowledged",String(authorized));form.append("file",file);try{const response=await fetch(apiUrl(`/voices/${voice.id}/performances`),{method:"POST",body:form});const payload=await response.json();if(!response.ok)throw new Error(errorMessage(payload)||"Performance creation failed.");onCreated(payload);setTranscript("");setFile(null);setAuthorized(false);setPreset(presets.find(item=>item!==payload.preset&&!performances.some(existing=>existing.preset===item))||"")}catch(err){setError(err.message)}finally{setBusy(false)}}; const remove=async item=>{if(!confirm(`Delete the ${item.preset} performance reference?`))return;const response=await fetch(apiUrl(`/voices/${voice.id}/performances/${item.preset}`),{method:"DELETE"});if(response.ok)onDeleted(item.preset)}; return <div className="modal"><form className="dialog performance-dialog" onSubmit={submit}><header><div><p className="label">VOICE IDENTITY / PERFORMANCE</p><h2>Direct {voice.name}</h2></div><button type="button" onClick={onClose} aria-label="Close"><Icon name="close"/></button></header><p>Record this same authorized speaker performing a delivery. Qwen will condition on the recording—not a decorative label.</p>{performances.length>0&&<div className="performance-list">{performances.map(item=><div key={item.id}><strong>{item.preset}</strong><span>{formatTime(item.duration_seconds)}</span><button type="button" onClick={()=>new Audio(assetUrl(item.preview_url)).play()}><Icon name="play"/></button><button type="button" className="danger" onClick={()=>remove(item)}><Icon name="trash"/></button></div>)}</div>}{available.length>0?<><label>PERFORMANCE<select value={preset} onChange={e=>setPreset(e.target.value)}>{available.map(item=><option key={item} value={item}>{item[0].toUpperCase()+item.slice(1)}</option>)}</select></label><label>EXACT TRANSCRIPT<textarea value={transcript} onChange={e=>setTranscript(e.target.value.slice(0,2000))} maxLength="2000" placeholder="Type exactly what is spoken in this performance recording"/></label><label className="upload"><Icon name="upload"/><span>{file?file.name:"Choose one performance recording"}<small>Clean single-speaker audio; 5–15 seconds recommended</small></span><input type="file" accept="audio/*" onChange={e=>setFile(e.target.files?.[0]||null)}/></label><label className="consent"><input type="checkbox" checked={authorized} onChange={e=>setAuthorized(e.target.checked)}/><span>I own this voice or have explicit permission to reproduce it.</span></label>{error&&<p className="form-error">{error}</p>}<footer><button type="button" onClick={onClose}>Done</button><button className="primary" disabled={busy}>{busy?"Preparing…":"Save performance"}</button></footer></>:<footer><button type="button" onClick={onClose}>Done</button></footer>}</form></div> }
